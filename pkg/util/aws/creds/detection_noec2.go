...
// HasAWSCredentialsInEnvironment returns true if static AWS credentials are present in environment variables.
...
// IsRunningOnAWS returns true when static credentials, IRSA, or container credential env vars are present.
// IMDS is not available without the ec2 build tag.
func IsRunningOnAWS(_ context.Context) bool {
	return HasAWSCredentialsInEnvironment() || HasAWSWorkloadIdentityInEnvironment() || HasAWSContainerCredentialsInEnvironment()
}
