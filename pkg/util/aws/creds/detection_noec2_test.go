...
func TestHasAWSCredentialsInEnvironment_BothSet(t *testing.T) {
	t.Setenv("AWS_ACCESS_KEY_ID", "AKID")
	t.Setenv("AWS_SECRET_ACCESS_KEY", "SECRET")
	assert.True(t, HasAWSCredentialsInEnvironment())
}

func TestHasAWSCredentialsInEnvironment_OnlyKeyID(t *testing.T) {
	t.Setenv("AWS_ACCESS_KEY_ID", "AKID")
	assert.False(t, HasAWSCredentialsInEnvironment())
}

func TestHasAWSCredentialsInEnvironment_NeitherSet(t *testing.T) {
	assert.False(t, HasAWSCredentialsInEnvironment())
}

func TestIsRunningOnAWSNoEc2_StaticEnvCreds(t *testing.T) {
	t.Setenv("AWS_ACCESS_KEY_ID", "AKID")
	t.Setenv("AWS_SECRET_ACCESS_KEY", "SECRET")
	assert.True(t, IsRunningOnAWS(context.Background()))
}
