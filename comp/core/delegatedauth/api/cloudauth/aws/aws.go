...
// getCredentials retrieves AWS credentials using the full standard credential chain.
// Delegates to resolveCredentials, which is build-tag-selected:
//   - ec2 build: AWS SDK LoadDefaultConfig (env -> web identity -> container -> IMDS -> SSO)
//   - non-ec2 build: lightweight SDK-free chain (env -> web identity -> container)
